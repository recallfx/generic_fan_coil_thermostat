...
class GenericFanCoilConfigFlow(config_entries.ConfigFlow, domain=DOMAIN):
    ...
    async def async_step_user(self, user_input=None):
        ...
        if user_input is not None:
            ...
            current_temp_entity = hass.states.get(
                user_input[CONF_CURRENT_TEMPERATURE_ENTITY_ID]
            )
            ...
            if not errors:
                ...
                await self.async_set_unique_id(
                    f"{user_input[CONF_CURRENT_TEMPERATURE_ENTITY_ID]}_{user_input[CONF_FAN_ENTITY_ID]}"
                )
            ...
            data_schema=vol.Schema(
                {
                    ...
                    vol.Optional(CONF_MIN_TEMP, default=DEFAULT_MIN_TEMP): vol.Coerce(
                        float
                    ),
                    vol.Optional(CONF_MAX_TEMP, default=DEFAULT_MAX_TEMP): vol.Coerce(
                        float
                    ),
                    vol.Optional(
                        CONF_TARGET_TEMP, default=DEFAULT_TARGET_TEMP
                    ): vol.Coerce(float),
                    vol.Optional(CONF_TEMP_STEP, default=DEFAULT_TEMP_STEP): vol.Coerce(
                        float
                    ),
                }
            ),
        ...
        options = {
            vol.Optional(
                ...
                default=self.config_entry.options.get(
                    CONF_COOLING_SWITCHES,
                    self.config_entry.data.get(CONF_COOLING_SWITCHES, []),
                ),
            ): selector.EntitySelector(
                selector.EntitySelectorConfig(
                    domain=["switch"],
            ...
            vol.Optional(
                ...
                default=self.config_entry.options.get(
                    CONF_HEATING_SWITCHES,
                    self.config_entry.data.get(CONF_HEATING_SWITCHES, []),
                ),
            ): selector.EntitySelector(
                selector.EntitySelectorConfig(
                    domain=["switch"],
            ...
            vol.Optional(
                ...
                default=self.config_entry.options.get(
                    CONF_MIN_TEMP,
                    self.config_entry.data.get(CONF_MIN_TEMP, DEFAULT_MIN_TEMP),
                ),
            ): vol.Coerce(float),
            vol.Optional(
                CONF_MAX_TEMP,
            ...
            vol.Optional(
                ...
                default=self.config_entry.options.get(
                    CONF_TARGET_TEMP,
                    self.config_entry.data.get(CONF_TARGET_TEMP, DEFAULT_TARGET_TEMP),
                ),
            ): vol.Coerce(float),
            vol.Optional(
                CONF_TEMP_STEP,
                default=self.config_entry.options.get(
                    CONF_TEMP_STEP,
                    self.config_entry.data.get(CONF_TEMP_STEP, DEFAULT_TEMP_STEP),
                ),
            ): vol.Coerce(float),
        }
